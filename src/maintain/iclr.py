...
def main() -> None:
    parser = argparse.ArgumentParser(description="维护入口：ICLR 抓取 + Supabase 同步。")
    ...
    cleanup_backend(backend_key="iclr", retention_days=args.retention_days, skip_cleanup=args.skip_cleanup)
    ...
    raw_path = str(args.raw_input or "").strip() or default_raw_path(
        f"iclr-openreview-{format_years_token(target_years)}",
        run_date,
    )
    ...
    init_cmd = [
        sys.executable,
        os.path.join(os.path.dirname(__file__), "init_iclr.py"),
        "--years",
    ...
    run_step("Maintain ICLR", init_cmd)
